...
void TcpConnection::send(const void* msg, int len) {
    send(std::string((const char*)msg, len));
}

// 发送string类型
void TcpConnection::send(const std::string& msg) {
    if (state_ == kConnected) {
        // 这种是对于单个reactor的情况 用户调用conn->send时
        // loop_即为当前线程
        if (loop_->isInLoopThread()) {
            sendInLoop(msg);
        } else {
            // 函数指针fp指向sendInLoop函数
            void (TcpConnection::*fp)(const std::string& msg) =
...
void TcpConnection::sendInLoop(const void* data, size_t len) {
    ...
    if (!channel_->isWriting() && outputBuffer_.readableBytes() == 0) {
        ...
        if (nwrote >= 0) {
            ...
            if (errno !=
                ...
                if (errno == EPIPE ||
                    errno == ECONNRESET) {  // SIGPIPE RESET
                    faultError = true;
                }
            }
        }
    ...
    if (!faultError && remaining > 0) {
        ...
        if (!channel_->isWriting()) {
            channel_
                ->enableWriting();  // 这里一定要注册channel的写事件
        ...
        if (n > 0) {
            ...
            if (outputBuffer_.readableBytes() == 0) {
                ...
                if (writeCompleteCallback_) {
                    // TcpConnection对象在其所在的subloop中
                    // 向pendingFunctors_中加入回调
...
void TcpConnection::handleError() {
    ...
    int       err = 0;
    if (::getsockopt(channel_->fd(), SOL_SOCKET, SO_ERROR, &optval,
                     &optlen) < 0) {
        err = errno;
    } else {
        err = optval;
